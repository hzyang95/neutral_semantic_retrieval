...
class RobertaGraphBasedModel(RobertaPreTrainedModel):

    def __init__(self, config, num_answer_type=3, num_hop=3, num_rel=2, no_gnn=False, edp=0.0,
                 sent_with_cls=False):
        super(RobertaGraphBasedModel, self).__init__(config)
    ...
    def forward(self, input_ids, input_mask, segment_ids, adj_matrix, graph_mask, sent_start,
        ...
        if gtem == 'ori':
            ...
            for i in range(bs):
                # print(sent_num[i])
                ll = int(sent_num[i])
                sent_sum_output_[i, :ll, :] = self.roberta(input_ids[i, :ll], token_type_ids=segment_ids[i, :ll],
                                                        attention_mask=input_mask[i, :ll])[1].squeeze(0)
