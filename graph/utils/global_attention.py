...
from utils.opennmt.sparse_activations import sparsemax
from utils.opennmt.misc import aeq, sequence_mask
...
class GlobalAttention(nn.Module):
    ...
    * Bahdanau Attention (mlp):
    ...
    def forward(self, source, memory_bank, memory_lengths=None, coverage=None):
        ...
        # over all the source hidden state
        align_vectors_ = torch.zeros(batch, target_l, source_l).to(align_vectors.device)
        for i in range(batch):
            align_vectors_[i, :int(memory_lengths[i]), :] = align_vectors[i, :int(memory_lengths[i]), :]

        # print(align_vectors_)
        # print(align_vectors_.size())
        c = torch.bmm(align_vectors_, memory_bank)
