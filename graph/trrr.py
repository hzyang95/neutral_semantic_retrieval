...
def train(model, tokenizer, dataset):
    ...
    scheduler = get_linear_schedule_with_warmup(optimizer, int(args.warmup_steps * t_total), t_total)
    aver_loss = 0.0
    for i in range(args.epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if (all_step) % args.log_step == 0:
                logging.info('all_step ' + str(all_step) +'/' + str(t_total)+ ' epoch ' + str(i) + ' step ' + str(step) + '/' + str(
...
if __name__ == "__main__":
    ...
    if args.raw:
        ...
        T = False
        dataset = {'train': processor.create_examples(args.train_data, 'train', T),
                   'dev': processor.create_examples(args.dev_data, 'dev', T),
                   'test': processor.create_examples(args.test_data, 'test', T)}
    ...
    #         dataset[i] = data
    #
    #     dataset['dev'] = DataLoader(dataset['dev'], batch_size=args.test_batch_size)
    #     dataset['test'] = DataLoader(dataset['test'], batch_size=args.test_batch_size)
    #     data_to_pickle(dataset, args.out_dir + "/features.pkl")
    # else:
    #     dataset = pickle_to_data("data/features.pkl")

    # if args.is_test:
    #     # processor = DataProcessor()
    #     # dt = processor.create_examples(args.test_data, 'test')
    #     # features = convert_examples_to_features(
    #     #     dt, [False, True], args.max_ques_length, args.max_seq_length, tokenizer, is_train=True)
    #     # data = get_batches(features, is_train=True)
    #     # dataset['test'] = DataLoader(data, batch_size=args.test_batch_size)
    #     # data_to_pickle(dataset, args.out_dir + "/features.pkl")
    #     model = load_torch_model(args.save, use_cuda=args.cuda)
    #     # print(type(model))
    #     # model = None
    #
    #     print(test(model, tokenizer, dataset['test']))
    # else:
    ...
    #                                     '_') + '_' + ind + '_'+str(min_loss)[2:6] +'_' + str(testres)[2:6]
    #     if not os.path.exists(save_path):
    #         os.makedirs(save_path)
    #         torch.save(best, save_path + '/model.pt')
